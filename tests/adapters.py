...
def run_multihead_self_attention(
    ...
    """
    print(f"Actual q_proj_weight shape: {q_proj_weight.shape}")
    print(f"Actual k_proj_weight shape: {k_proj_weight.shape}")
    print(f"Actual v_proj_weight shape: {v_proj_weight.shape}")
    print(f"Actual o_proj_weight shape: {o_proj_weight.shape}")
    print(f"d_model: {d_model}, num_heads: {num_heads}")
    
    from cs336_basics.model import MultiHeadAttention
    mha = MultiHeadAttention(d_model=d_model, num_heads=num_heads)
    mha.load_state_dict({"q_proj": q_proj_weight,
...
def run_multihead_self_attention_with_rope(
    ...
    mha_rope = MultiHeadAttentionWithRoPE(d_model=d_model, num_heads=num_heads, 
                                           max_seq_len=max_seq_len, theta=theta)
    mha_rope.load_state_dict({"q_proj": q_proj_weight,
                               "k_proj": k_proj_weight,
                               "v_proj": v_proj_weight,
                               "o_proj": o_proj_weight}, strict=False)
...
def run_get_batch(
    ...
    """
    from cs336_basics.nn_utils import dataloader
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    gradient_clipping(parameters, max_norm=max_l2_norm)
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.nn_utils import cosine_lr_schedule_with_warmup
    return cosine_lr_schedule_with_warmup(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
